...
main()
{
  ...
  if $show_powerline; then
    ...
    tmux set-option -g status-left "#($current_dir/mode-left-color.sh before)#{?client_prefix,#[bg=${yellow}],} ${left_icon}#($current_dir/mode-left-color.sh after)#{?client_prefix,#[fg=${yellow}],}${left_sep}"
    powerbg=${gray}
  else
